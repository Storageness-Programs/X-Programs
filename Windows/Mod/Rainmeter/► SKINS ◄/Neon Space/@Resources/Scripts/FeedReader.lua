   ...
   elseif (Raw ~= Feeds[f].Raw) or UserInput then Feeds[f].Raw = Raw
	...
	--	TRIM THE FEED TITLES FOR RAINMETER FORUMS
		if Feeds[f].Title == 'Rainmeter Forums' then 
			...
			Item.Title  = RawItem:match('<title.->.-(%S.-)</title>') or nil	
		end		
...
-- print("Age in mins = "..( (timeNow-Item.Date)/60 ) )	
...
function Output()
  ...
  if Error then
   ...
   -- SET VARIABLES
      local numNew = 0
      for i = 1, MinItems do
            numNew = numNew + Queue['Item'..i..'New']
      end
	
--	print("num new "..numNew)
	  SKIN:Bang('!SetVariable', "numNewItems"..f, numNew)
   	  
   local VariablePrefix = SELF:GetOption('VariablePrefix', '')
   for k, v in pairs(Queue) do
      SKIN:Bang('!SetVariable', VariablePrefix..k, v)
   end
	SKIN:Bang('!SetVariable', "FeedTitle"..f, Feed.Title)
...
function IdentifyDate(s, t)
   ...
   if (Date.year and Date.month and Date.day) then
      ...
      if (Date.Offset) and (Date.Offset ~= '') then 
         ...
         -- Date.Offset = LocalOffset -- version that worked before 15th October
		 	Date.Offset = 0
      end
   -- print("Local time posted = "..(Date.hour+( (LocalOffset-Date.Offset)/3600)).."-"..Date.min.."-"..Date.sec)	 	  
